...
public class Printer implements Printable, ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		/*
		 * PrinterJob job = PrinterJob.getPrinterJob(); job.setPrintable(this);
		 * boolean ok = job.printDialog(); if (ok) { try { job.print(); } catch
		 * (PrinterException ex) { /* The job did not successfully complete } }
		 */
	...
	public void printBill(Bill bill) {
		 ...
		 * sb.append(order.getOrderItemID() + "    " + order.getQuantity() +
		 * "    " + order.getCost() + "<br>"); } sb.append(smallLine);
		 * 
		 * double amountFood = bill.getOrders().stream().filter(b ->
		 * b.getType().equals(ITEM_TYPE_FOOD)) .mapToDouble(b ->
		 * b.getCost()).sum(); double amountBar =
		 * bill.getOrders().stream().filter(b ->
		 * b.getType().equals(ITEM_TYPE_BAR)) .mapToDouble(b ->
		 * b.getCost()).sum(); System.out.println(amountBar + " " + amountFood);
		 * 
		 ...
		 * " : " + taxDetail.getTaxValue() / 100 * amountBar + "<br>"); totalTax
		 * += taxDetail.getTaxValue() / 100 * amountBar; } } for(Orders order :
		 * bill.getOrders()){ if(order.getType().equals(ITEM_TYPE_FOOD))
		 * sb.append(order.getOrderItemID() + "    " + order.getQuantity() +
		 * "    " + order.getCost() + "<br>"); } sb.append(smallLine);
		 * 
		 ...
		 * sb.append(bigLine); sb.append("<br>"); grandTotal = totalTax +
		 * amountBar + amountFood; sb.append("Grand Total : " + grandTotal +
		 * "<br>"); sb.append("Thank You !!<br>");
		 */
	...
	/*
	 * public void writeToPDF(String data) throws IOException{ PDDocument
	 * document = new PDDocument(); PDPage page = new PDPage();
	 * document.addPage( page ); //PDFont font = PDType1Font.HELVETICA_BOLD;
	 * PDFont font = PDType1Font.TIMES_ROMAN; PDPageContentStream contentStream
	 * = new PDPageContentStream(document, page); contentStream.beginText();
	 * contentStream.setFont( font, 12 );
	 * contentStream.moveTextPositionByAmount( 100, 700 );
	 * contentStream.drawString( data ); contentStream.endText();
	 * 
	 * contentStream.close();
	 * 
	 * // Save the results and ensure that the document is properly closed:
	 * document.save( "/home/ameya/Desktop/Bill.pdf"); document.close(); }
	 */
	...
	public static void main(String[] args) throws PrinterException, Exception {
		...
		for (PrintService printer : printServices) {
			...
			if (printer.getName().equals("\\\\Server\\billing")) {
